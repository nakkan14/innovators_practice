...
# 表示するデータを読み込み
df = pd.read_csv('20231015_SUUMO_和歌山市賃貸_最寄り施設_詳細_改訂v2.csv')
...
# 地区ブロックの設定
block_code = 5
dataframes = [df,  df_school, df_nursery, df_kindergarten, df_lesson]
filtered_dataframes = [df[df['地区_コード'] == block_code] for df in dataframes]
df,  df_school, df_nursery, df_kindergarten, df_lesson= filtered_dataframes

df_park = df_park[df_park['地区_ブロック'] == '東部地域']
df_hospital = df_hospital[df_hospital['Block'] == '東部地域']
df_supermarket = df_supermarket[df_supermarket['Block'] == '東部地域']
